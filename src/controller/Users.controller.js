...
const MiddlewarRegister =(req,res,next)=>{
    ...
    if(!username||!email||!password){
        return render('profile',{error:"Please Enter the form!!"})
    }
    if(password.length < 6){
        return('profile',{error:'Password must be over 6 digits'})
    }
    next();
...
const registerUser = async (req, res) => { 
    try {
        ...
        if (existUser.length > 0) { 
           return  res.render('profile',{ error: 'This email already exist' });
        }
...
module.exports ={
    GetProfile,registerUser,LoginUser,MiddlewarRegister,AuthenticateUser
}